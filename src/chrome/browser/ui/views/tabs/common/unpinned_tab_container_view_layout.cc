...
namespace {
// Avora: small gap between tabs for a compact but readable strip.
...
bool UnpinnedTabContainerViewLayout::IsChildVisibleInContainer(
  ...
  // Avora: a favorite's tab is represented by its tile in the favorites row,
  // so it never also gets a row in the daily list.  This is decided at layout
  // time rather than in response to a tab strip event because the tab is
  // marked just after Navigate() has already inserted it and notified
  // observers; an event-driven check would run a beat too early and leave the
  // row on screen until some later, unrelated change swept it away.
  if (const TabView* tab_view = views::AsViewClass<TabView>(child)) {
    if (const tabs::TabInterface* tab = tab_view->GetTabInterface()) {
      tabs::TabInterface* live_tab = tab->GetHandle().Get();
      if (live_tab && avora::IsFavoriteTab(live_tab->GetContents())) {
        return false;
      }
    }
  }

...
namespace avora {
class AvoraLinkPreviewTabIndicator;
}  // namespace avora
...
class TabView : public views::View,
 ...
 private:
  ...
  // Avora: a link preview parked on this tab is invisible from every other
  // tab, so the row carries a marker while one is waiting behind it.
  void OnAvoraLinkPreviewStateChanged(content::WebContents* tab);
  void UpdateAvoraLinkPreviewIndicator();
  ...
  // Avora: only created for vertical tab strips, which is the only strip
  // Avora ships.
  raw_ptr<avora::AvoraLinkPreviewTabIndicator> avora_preview_indicator_ =
      nullptr;
  ...
  base::CallbackListSubscription avora_preview_state_subscription_;

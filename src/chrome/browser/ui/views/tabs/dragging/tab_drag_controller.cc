...
TabDragController::GetDragTargetForPoint(gfx::Point point_in_screen) {
  ...
  if (local_window && CanAttachTo(local_window)) {
    ...
    if (destination_tab_strip) {
      ...
      // Avora: If the cursor is anywhere within the TabStripView (which
      // includes the favorites grid, pinned section, and daily tabs), keep
      // the tab attached to prevent it from detaching into a new window.
      if (browser_view->tab_strip_view()->GetBoundsInScreen().Contains(
              point_in_screen)) {
        // If the cursor is within the daily (unpinned) section, resolve the
        // TabDragTarget so the standard DraggedTabsContainer mechanism
        // handles visual drag movement and model reordering.
        auto* inner_strip = static_cast<TabStripView*>(
            browser_view->tab_strip_view()->GetTabStripView());
        views::ScrollView* unpinned_scroll =
            inner_strip ? inner_strip->unpinned_tabs_scroll_view() : nullptr;
        if (unpinned_scroll &&
            unpinned_scroll->GetBoundsInScreen().Contains(point_in_screen)) {
          if (TabDragTarget* candidate =
                  GetTabDragPointResolver()->GetDragTarget(
                      *browser_view, point_in_screen)) {
            return std::tuple(Liveness::kAlive, destination_tab_strip,
                              candidate);
          }
        }
        // Over favorites/pinned area — keep attached without in-section
        // reordering. Avora drop logic in ContinueDrag/StoppedDragging
        // handles visual feedback and the final drop action.
        return std::tuple(Liveness::kAlive, destination_tab_strip, nullptr);
      }
...
TabDragController::DetachIntoNewBrowserAndRunMoveLoop(
  ...
  dragged_widget->GetNativeWindow()->SetProperty(
      ...
      attached_context_->GetWidget()->GetNativeWindow()->GetWeakPtrAsWindow());

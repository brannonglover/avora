...
namespace {
// Avora: spacing between the + icon and the "New Tab" label text.
constexpr int kButtonWithLabelPadding = 10;
// Avora: left inset that centers the + icon with tab favicons.
// Tabs are inset by kVerticalTabStripHorizontalPadding (6) + the
// vertical-tab content inset (10) = 16px from the strip edge.
constexpr int kButtonWithLabelLeftInset = 16;
...
void TabStripFlatEdgeButton::OnPaintBackground(gfx::Canvas* canvas) {
  if (paint_transparent_for_glass_) {
    return;
  }
...
void TabStripFlatEdgeButton::UpdateLabel(bool should_show) {
  ...
  if (should_show == GetText().empty()) {
    ...
    SetInsets(should_show
                  ? gfx::Insets::TLBR(0, kButtonWithLabelLeftInset, 0,
                                      kButtonWithLabelPadding)
                  : gfx::Insets());
  }

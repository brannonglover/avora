...
// Avora: ToolbarCycleFocusWithBookmarkBar removed.  It covered focus cycling
// with the bookmark bar already showing at window open, and the bar can never
// show -- IDC_SHOW_BOOKMARK_BAR is disabled and BookmarkBarController returns
// early (see browser_defaults::bookmarks_enabled in
// chrome/browser/defaults.cc).  ToolbarCycleFocus above still covers the
// toolbar-only path.

...
bool BrowserRootView::OnMousePressed(const ui::MouseEvent& event) {
  gfx::Point point_in_browser_view = event.location();
  views::View::ConvertPointToTarget(this, browser_view_,
                                    &point_in_browser_view);
  if (browser_view_->TryHandleSpaceSwitchMousePress(point_in_browser_view,

...
class AvoraSidebarView;
class AvoraSpacesBarView;
namespace avora {
...
class BrowserView : public BrowserWindow,
 ...
 public:
  ...
  bool IsPointInSpaceSwitchRegion(
      const gfx::Point& point_in_browser_view_coords) const;
  bool TryHandleSpaceSwitchMousePress(
      const gfx::Point& point_in_browser_view_coords,
      int changed_button_flags);

  // Per-window active Space state.  Exposed so VerticalTabStripRegionView can
  // pass it through to TabStripView and its child Avora views.
  avora::WindowSpaceState* GetWindowSpaceState() const {
 ...
 private:
  ...
  // Avora sidebar header (nav row + address chip + space name) and footer.
  raw_ptr<AvoraSidebarView> avora_sidebar_header_ = nullptr;
  raw_ptr<AvoraSpacesBarView> avora_spaces_bar_ = nullptr;

...
IN_PROC_BROWSER_TEST_F(BrowserViewTest, FullscreenClearsFocus) {
  ...
  // Avora: the IDC_SHOW_BOOKMARK_BAR assertions that upstream interleaved
  // here are dropped -- the command is permanently disabled (see
  // browser_defaults::bookmarks_enabled in chrome/browser/defaults.cc).
  // This test's subject, top-view visibility across fullscreen modes, is
  // unaffected.
...
// Avora: FullscreenShowBookmarkBar removed.  Its entire subject was whether
// the bookmark bar shows or hides across fullscreen modes, and the bar can
// never be shown -- BookmarkBarController returns early and
// chrome::ToggleBookmarkBar() is a no-op (see
// browser_defaults::bookmarks_enabled in chrome/browser/defaults.cc).
// Verified against a baseline build with bookmarks_enabled=true: this was
// the only test in this binary that the kill switch regressed.

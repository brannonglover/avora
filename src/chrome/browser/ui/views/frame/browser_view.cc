...
BrowserView::BrowserView(BrowserWindowInterface* browser)
  ...
  // Avora: hand the Live Folder subsystem the process-wide encryption service
  // before any sidebar view can construct it.  //chrome/browser/avora is a
  // dependency of //chrome/browser, so it cannot reach g_browser_process
  // itself.
  avora::LiveFolderService::SetOSCryptAsync(g_browser_process->os_crypt_async());
  ...
  if (vertical_tab_strip_state_controller) {
...

    {
      ...
      avora_sidebar_header_ = AddChildView(std::make_unique<AvoraSidebarView>(
          ...
          base::BindRepeating(
              [](BrowserView* bv, const std::string& extension_id) {
                bv->ShowAvoraExtensionPopup(extension_id);
              },
              base::Unretained(this)),
          base::BindRepeating(
              [](BrowserView* bv) {
                bv->NavigateAvoraAddressBar(u"chrome://extensions",
                                            /*new_tab=*/true);
              },
              base::Unretained(this)),
          base::BindRepeating(
              [](BrowserView* bv) {
                bv->NavigateAvoraAddressBar(
                    u"https://chromewebstore.google.com",
                    /*new_tab=*/true);
              },
              base::Unretained(this)),
          base::BindRepeating(
    ...
    for (views::View* region :
         {static_cast<views::View*>(avora_sidebar_header_),
          static_cast<views::View*>(vertical_tab_strip_region_view_),
          static_cast<views::View*>(avora_spaces_bar_)}) {
      if (region) {
        gesture_regions.push_back(region);
      }
    }
    avora_space_gesture_controller_ =
        std::make_unique<avora::AvoraSpaceGestureController>(
            avora_window_space_state_.get(), std::move(gesture_regions));

    // Quick-nav overlay (CMD+T).
    avora_quick_nav_ =
        AddChildView(std::make_unique<avora::AvoraQuickNavView>(
            browser_.get(),
            avora_window_space_state_.get(),
...
int BrowserView::NonClientHitTest(const gfx::Point& point) {
  ...
  // Avora: the dark-chrome background surrounding the inset content card is
  // the window's drag handle.  Any point that wasn't claimed by an
  // interactive view above and falls outside the content card (and the side
  // panel, if visible) is treated as caption area.  On macOS the caller
  // (BrowserFrameViewMac) converts HTNOWHERE → HTCAPTION, which gives us
  // both window dragging and double-click-to-zoom for free.
  if (ShouldDrawVerticalTabStrip() && !IsFullscreen()) {
    bool in_interactive_child = false;

    if (multi_contents_view_ && multi_contents_view_->GetVisible() &&
        multi_contents_view_->bounds().Contains(point_in_browser_view_coords)) {
      in_interactive_child = true;
    }
    if (!in_interactive_child && side_panel_ && side_panel_->GetVisible() &&
        side_panel_->bounds().Contains(point_in_browser_view_coords)) {
      in_interactive_child = true;
    }

    if (!in_interactive_child) {
      return HTNOWHERE;
    }
  }
...
bool BrowserView::IsPointInSpaceSwitchRegion(
    const gfx::Point& point_in_browser_view_coords) const {
  if (!ShouldDrawVerticalTabStrip()) {
    return false;
  }

  auto point_in_view = [&](views::View* view) -> std::optional<gfx::Point> {
    if (!view || !view->GetVisible()) {
      return std::nullopt;
    }
    gfx::Point point = point_in_browser_view_coords;
    views::View::ConvertPointToTarget(this, view, &point);
    return point;
  };

  if (avora_sidebar_header_) {
    if (std::optional<gfx::Point> point = point_in_view(avora_sidebar_header_);
        point && avora_sidebar_header_->HitTestPoint(*point)) {
      return true;
    }
  }

  if (vertical_tab_strip_region_view_) {
    if (std::optional<gfx::Point> point =
            point_in_view(vertical_tab_strip_region_view_);
        point &&
        vertical_tab_strip_region_view_->HitTestPoint(*point)) {
      return true;
    }
  }

  if (avora_spaces_bar_) {
    if (std::optional<gfx::Point> point = point_in_view(avora_spaces_bar_);
        point && avora_spaces_bar_->HitTestPoint(*point)) {
      return true;
    }
  }
...
bool BrowserView::TryHandleSpaceSwitchMousePress(
    const gfx::Point& point_in_browser_view_coords,
    int changed_button_flags) {
  if (!avora_space_gesture_controller_ ||
      !IsPointInSpaceSwitchRegion(point_in_browser_view_coords)) {
    return false;
  }

  if (changed_button_flags == ui::EF_FORWARD_MOUSE_BUTTON) {
    avora_space_gesture_controller_->ActivateAdjacentSpace(true);
    return true;
  }
  if (changed_button_flags == ui::EF_BACK_MOUSE_BUTTON) {
    avora_space_gesture_controller_->ActivateAdjacentSpace(false);
    return true;
  }
  return false;
}
...
void BrowserView::ShowAvoraExtensionPopup(const std::string& extension_id) {
  auto* profile = GetProfile();
  auto* registry = extensions::ExtensionRegistry::Get(profile);
  if (!registry) return;
  const auto* ext = registry->enabled_extensions().GetByID(extension_id);
  if (!ext) return;
  auto* action_mgr = extensions::ExtensionActionManager::Get(profile);
  if (!action_mgr) return;
  auto* action = action_mgr->GetExtensionAction(*ext);
  if (!action) return;
  auto* web_contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  if (!web_contents) return;
  int tab_id = sessions::SessionTabHelper::IdForTab(web_contents).id();
  if (!action->HasPopup(tab_id)) return;
  GURL popup_url = action->GetPopupUrl(tab_id);

  auto host = extensions::ExtensionViewHostFactory::CreatePopupHost(
      *ext, popup_url, browser());
  if (!host) return;

  views::View* anchor =
      avora_sidebar_header_ ? avora_sidebar_header_->manage_button() : nullptr;
  if (!anchor) return;

  ExtensionPopup::ShowPopup(
      browser(), std::move(host), views::BubbleAnchor(anchor),
      views::BubbleBorder::TOP_LEFT, PopupShowAction::kShow,
      base::BindOnce([](extensions::ExtensionHost*) {
        // Activate the popup widget right after it's shown so that it becomes
        // the active widget in the tree.  This prevents the immediate close
        // caused by the main browser window being active after the manage
        // bubble tears down, while still allowing click-outside-to-close to
        // work normally afterwards.
        auto* popup = ExtensionPopup::last_popup_for_testing();
        if (popup && popup->GetWidget()) {
          popup->GetWidget()->Activate();
        }
      }));
}

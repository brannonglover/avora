...
struct BrowserWindowCreateParams {
  ...
  // Avora: durable window identity that survives session restore.  Generated
  // once for a new window, persisted through kCommandAddWindowExtraData, and
  // restored from SessionWindow::extra_data.
  std::string avora_window_guid;

  // Avora: the active Space at the time the window was last persisted.
  // Used during session restore to reinitialise WindowSpaceState with the
  // same Space the user was in when the session ended.
  std::string avora_active_space_id;

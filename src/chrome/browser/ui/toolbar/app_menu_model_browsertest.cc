...
IN_PROC_BROWSER_TEST_F(AppMenuModelTest, Basics) {
  ...
  // Avora: there is no bookmark submenu to choose from.  Upstream used it here
  // to check that a submenu activation routes back to the delegate; that part
  // of the coverage moves to the "More tools" submenu below, which is the
  // nearest equivalent that still exists.
  EXPECT_FALSE(
      model.GetIndexOfCommandId(AppMenuModel::kBookmarksMenuPlaceholder)
          .has_value());

  size_t more_tools_index =
      model.GetIndexOfCommandId(AppMenuModel::kMoreToolsMenuPlaceholder)
          .value();
  EXPECT_GT(more_tools_index, 0u);
  ui::MenuModel* more_tools_model = model.GetSubmenuModelAt(more_tools_index);
  ASSERT_TRUE(more_tools_model);
  more_tools_model->MenuWillShow();
  ASSERT_GT(more_tools_model->GetItemCount(), 0u);

  more_tools_model->ActivatedAt(0);
  EXPECT_TRUE(more_tools_model->IsEnabledAt(0));

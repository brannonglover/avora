...
void AppMenuModel::Build() {
  ...
  SetElementIdentifierAt(GetIndexOfCommandId(IDC_SHOW_DOWNLOADS).value(),
                         kDownloadsMenuItem);
  // Avora: no "Bookmarks and lists" submenu.  Disabling the commands (see
  // browser_defaults::bookmarks_enabled in chrome/browser/defaults.cc) would
  // only grey these items out, so the submenu is not built at all.  This also
  // removes "Import Bookmarks and Settings..." (IDC_IMPORT_SETTINGS), which
  // upstream nests inside it -- on macOS that importer only ever finds Safari
  // and Firefox, never Chrome, Edge or Arc.  Avora's own importer is
  // AvoraImportDialogView (chrome/browser/ui/views/avora/).
  //
  // |bookmark_sub_menu_model_| is intentionally left null; its only reader is
  // the bookmark_sub_menu_model() accessor, which now returns nullptr.

...
void AddInfoBarsIfNecessary(BrowserWindowInterface* browser,
  ...
  bool should_display_bubble =
      !is_web_app && HasPendingUncleanExit(browser->GetProfile());

  // Avora: restore the previous session outright rather than offering to.  A
  // crash should cost the user no more of their daily tabs than a quit does,
  // and the bubble puts that behind a click the user has to notice and take
  // before they touch anything else.  The bubble stays as the fallback for
  // when the crash-loop guard declines to restore automatically.
  if (should_display_bubble &&
      avora::RestorePreviousSessionAfterCrash(browser)) {
    should_display_bubble = false;
  }

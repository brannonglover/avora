...
// Avora: replaces upstream's BookmarkAllTabsUpdatesOnTabStripChanges,
// BookmarkTabEnabledWhenBookmarkModelIsAlreadyLoaded and
// BookmarkBarSubmenuCommandsExecuteCorrectly.  All three asserted that
// bookmark commands become enabled; Avora disables the bookmark feature
// wholesale (see browser_defaults::bookmarks_enabled in
// chrome/browser/defaults.cc), so the commands are never enabled and the
// tests have no behavior left to exercise.  This guards the kill switch
// instead: every bookmark command stays disabled, including with a loaded
// BookmarkModel and more than one tab -- the exact conditions that used to
// enable them.
IN_PROC_BROWSER_TEST_F(BrowserCommandControllerBrowserTest,
                       BookmarkCommandsAreDisabled) {
  ...
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_SHOW_BOOKMARK_BAR));
  EXPECT_FALSE(command_controller->IsCommandEnabled(IDC_BOOKMARK_BAR_SUBMENU));
  EXPECT_FALSE(
      command_controller->IsCommandEnabled(IDC_SHOW_BOOKMARK_MANAGER));
}

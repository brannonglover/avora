...
#if !BUILDFLAG(IS_ANDROID)
  // Avora: confirm with the Avora toast in the upper-right of the content
  // area.  Every copy-URL entry point -- the Cmd-Shift-C accelerator, the Edit
  // menu, the app menu action and the tab context menu -- funnels through
  // here, so they all get the same confirmation.  Chromium's own
  // bottom-anchored "Link copied" toast is deliberately not raised.
  if (BrowserView* const browser_view =
          BrowserView::GetBrowserViewForBrowser(browser)) {
    browser_view->ShowAvoraToast(u"Copied URL");
  }
#endif
}

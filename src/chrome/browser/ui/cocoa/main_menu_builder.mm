  ...
  NSMenuItem* item =
      Item(IDS_EDIT_MENU_MAC)
          ...
          .submenu({
              ...
              // Avora: copy the active tab's URL (Cmd-Shift-C).  The shift is
              // carried by the uppercase key equivalent, not the flags.
              Item(IDS_COPY_URL)
...
NSMenuItem* BuildViewMenu(NSApplication* nsapp,
  ...
  NSMenuItem* item =
      Item(IDS_VIEW_MENU_MAC)
          .tag(IDC_VIEW_MENU)
          .submenu({
              // Avora: no "Always Show Bookmarks Bar" -- there is no
              // bookmark bar to show.  See browser_defaults::bookmarks_enabled
              // in chrome/browser/defaults.cc.
...
NSMenu* BuildMainMenu(NSApplication* nsapp,
  ...
  for (auto* builder : {
           ...
           &BuildViewMenu,
           &BuildHistoryMenu,
           // Avora: no Bookmarks menu.  Avora has no bookmark model --
           // imported links live in the sidebar's Imported section (see
           // chrome/browser/avora/avora_imported_link_store.h), so the
           // upstream Bookmarks menu would only surface commands that do
           // nothing here.
           &BuildPeopleMenu,
           &BuildTabMenu,
           &BuildWindowMenu,

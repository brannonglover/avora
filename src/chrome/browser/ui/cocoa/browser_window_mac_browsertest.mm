...
IN_PROC_BROWSER_TEST_F(BrowserWindowMacTest, MenuCommandsAfterDestroy) {
  ...
  // Avora: the Bookmarks menu no longer exists (see
  // chrome/browser/ui/cocoa/main_menu_builder.mm).  Any main-menu item backed
  // by commandDispatch: exercises the same path, so this uses File > Print.
  NSMenuItem* menu_item = [[[NSApp mainMenu] itemWithTag:IDC_FILE_MENU]
      .submenu itemWithTag:IDC_PRINT];

  EXPECT_TRUE(window);
  EXPECT_TRUE(menu_item);

  ui_test_utils::BrowserDestroyedObserver observer(browser());
  chrome::CloseAllBrowsersAndQuit();
  observer.Wait();

  EXPECT_EQ([menu_item action], @selector(commandDispatch:));
  ...
  EXPECT_TRUE([window validateUserInterfaceItem:menu_item]);
}
...
IN_PROC_BROWSER_TEST_F(BrowserWindowMacTest, DisableCommandsWhenSheetAttached) {
  ...
  // Avora: IDC_BOOKMARK_ALL_TABS dropped -- there is no Bookmarks menu to
  // retrieve it from.  IDC_PRINT / IDC_SAVE_PAGE still cover both the
  // enabled and sheet-attached cases below.
  ASSERT_TRUE(

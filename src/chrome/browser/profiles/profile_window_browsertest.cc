...
IN_PROC_BROWSER_TEST_F(ProfileWindowBrowserTest, GuestAppMenuLacksBookmarks) {
  ...
  // Avora: no profile has a bookmark menu -- the submenu is never built (see
  // chrome/browser/ui/toolbar/app_menu_model.cc).  Upstream expected the
  // normal profile to have one and only guest to lack it; here both lack it,
  // so this now guards that the removal holds for the normal profile too.
  AppMenuModel model_normal_profile(&accelerator_handler, browser());
  model_normal_profile.Init();
  EXPECT_FALSE(model_normal_profile

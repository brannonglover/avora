...
- (void)setLastProfile:(Profile*)profile {
  ...
  // Avora: the main menu has no Bookmarks item (see
  // chrome/browser/ui/cocoa/main_menu_builder.mm), so |bookmarkItem| is nil
  // and no BookmarkMenuBridge is created.  Messaging nil is a no-op, but the
  // bridge itself CHECKs its root menu, so the construction below is guarded.
  ...
  if (bookmarkItem) {
    ...
    if (!entry || !entry->GetProfile()) {
      ...
      // bridge on the original profile, to prevent bugs WRT profile lifetime.

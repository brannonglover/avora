...
void SessionServiceBase::BuildCommandsForBrowser(
  ...
  // Avora: persist the durable window GUID and active Space ID so session
  // rebuilds don't drop them.
  if (const auto* avora_data = avora::AvoraWindowSessionData::Get(
          browser->GetUnownedUserDataHost())) {
    if (!avora_data->window_guid().empty()) {
      command_storage_manager()->AppendRebuildCommand(
          sessions::CreateAddWindowExtraDataCommand(
              browser->GetSessionID(),
              avora::AvoraWindowSessionData::kWindowGuidKey,
              avora_data->window_guid()));
    }
    if (!avora_data->active_space_id().empty()) {
      command_storage_manager()->AppendRebuildCommand(
          sessions::CreateAddWindowExtraDataCommand(
              browser->GetSessionID(),
              avora::AvoraWindowSessionData::kActiveSpaceIdKey,
              avora_data->active_space_id()));
    }
  }

...
class SessionRestoreImpl : public BrowserCollectionObserver {
 ...
 private:
  ...
  BrowserWindowInterface* CreateRestoredBrowser(
    ...
    // Avora: restore per-window GUID and active Space from session data.
    {
      auto guid_it = extra_data.find(
          avora::AvoraWindowSessionData::kWindowGuidKey);
      if (guid_it != extra_data.end() && !guid_it->second.empty()) {
        params.avora_window_guid = guid_it->second;
      }
      auto space_it = extra_data.find(
          avora::AvoraWindowSessionData::kActiveSpaceIdKey);
      if (space_it != extra_data.end() && !space_it->second.empty()) {
        params.avora_active_space_id = space_it->second;
      }
    }
